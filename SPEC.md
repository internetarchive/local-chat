...
strings); if that fails, treated as a single plain-string Starter. The
`.starters` property accepts an array of strings directly too (filtered
to string items, no `JSON.parse` round trip); a plain string set via the
property is parsed the same way the attribute is. Purely declarative —
never generated by the model, so there's no cap to configure.
